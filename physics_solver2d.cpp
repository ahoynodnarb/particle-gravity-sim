...
void p_sim::check_collision(particle2d &p, particle2d &q) {
  // points to p
  vec2 r = p.x - q.x;
...
void p_sim::apply_gravity(particle2d &p, particle2d &q) {
  vec2 r = p.x - q.x;
...
void physics_solver2d::add_particle(std::unique_ptr<particle2d> p) {
  particles_.push_back(std::move(p));
}
const std::vector<std::unique_ptr<particle2d>> &
physics_solver2d::get_particles() const {
  return particles_;
}
void physics_solver2d::step() {
